...
function findCaterer(budget, people){
  ...
  if (people === 0) {
    return -1
  }else if ((newBudget / people) < 15) {
    return -1
  }
  
  if (people > 60) {
    discount = (20/100) * budget
    newBudget = newBudget + discount
  }
  
  const maximumCost = Math.ceil((newBudget / people))
  
  if (maximumCost >= 30) {
    return 3
  }else if (maximumCost >= 20) {
    return 2
  }else if (maximumCost >= 15) {
    return 1
  }
  return 0;
}